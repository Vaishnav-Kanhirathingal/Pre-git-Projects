...
class ForageableViewModel(private val foragableDao: ForageableDao) : ViewModel() {

    //create a property to set to a list of all forageables from the DAO
    val allForageables :LiveData<List<Forageable>> = foragableDao.getAllForagables().asLiveData()
    ...
    fun getForageableById(id:Long):Flow<Forageable>{
        return foragableDao.getForageableById(id)
    }
    ...
    fun addForageable(
        ...
        viewModelScope.launch {
            //call the DAO method to add a Forageable to the database within it
            foragableDao.insertForageable(forageable)
        }
    }
    ...
    fun updateForageable(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            foragableDao.updateForageable(forageable)
        }
    }
    ...
    fun deleteForageable(forageable: Forageable) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            foragableDao.deleteForageable(forageable)
        }
    }
...
//create a view model factory that takes a ForageableDao as a property and creates a ForageableViewModel
class ForageableViewModelFactory(private val foragableDao: ForageableDao) :
    ViewModelProvider.Factory {
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(ForageableViewModel::class.java)) {
            @Suppress("UNCHECKED_CAST")
            return ForageableViewModel(foragableDao) as T
        }
