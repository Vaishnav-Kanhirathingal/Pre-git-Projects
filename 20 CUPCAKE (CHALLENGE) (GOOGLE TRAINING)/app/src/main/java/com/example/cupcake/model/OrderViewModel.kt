...
class OrderViewModel : ViewModel() {
    private val TAG = "PROJECT TAG - ORDER VIEW MODEL"
    private var isSpecialFlavor = false
    var userName = ""
    var isUserNameANumber = false
    ...
    var dateOptions: List<String> = getPickUpOptions()
    ...
    fun setFlavor(flavourGiven: String, isASpecialFlavor: Boolean) {
        Log.d(TAG,"setFlavor executed, isASpecialFlavor = ${isSpecialFlavor}")
        _flavor.value = flavourGiven
        isSpecialFlavor=isASpecialFlavor
    ...
    fun getPickUpOptions(): List<String> {
        ...
        if (isSpecialFlavor) {
            Log.d(TAG,"special flavor options generated = ${isSpecialFlavor}")
            repeat(4) {
    ...
    private fun updatePrice() {
        ...
        Log.d(
            TAG, "PRICE.VALUE = ${price.value}, _PRICE.VALUE = ${_price.value}" +
